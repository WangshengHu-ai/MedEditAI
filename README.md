...
## 📦 Troubleshooting Unsigned Builds ("App is Damaged")

Since the installer package generated by GitHub Actions uses **Ad-Hoc / Unsigned** codesigning (bypassing a $99 paid Apple Developer subscription), macOS Gatekeeper will block it upon download, displaying a security dialog claiming the app **"is damaged and cannot be opened" (已损坏，无法打开)**.

This is simply macOS applying a "quarantine" flag to files downloaded from web browsers. You can safely strip this quarantine attribute by running the following command in your Mac Terminal:

```bash
# Unzip the package first:
unzip MedEditAI-macOS.zip

# Clear the macOS quarantine metadata flag:
xattr -cr MedEditAI.app
```

Once executed, double-click `MedEditAI.app` to launch the app instantly.
